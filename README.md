...
| 技能                | 说明                                                                                                         |
| ------------------- | ------------------------------------------------------------------------------------------------------------ |
| **zx-blind**        | 盲区扫描 — 读指定日期 AI 对话，照出一个思维盲区，并用微信读书章节精准补位                                    |
| **zx-book**         | 拆书 — 把整本书转成读者可运行的解释路径，不做章节摘要                                                        |
| **zx-card**         | 内容铸卡 — 将内容转为 PNG 视觉卡片（长图 `-l`、信息图 `-i`、多卡 `-m`、视觉笔记 `-v`、漫画 `-c`、白板 `-w`、大字 `-b`） |
| **zx-classic**      | 古文笺 — 文言文逐字注解、章节解读和长 PNG 排版                                                              |
| **zx-constraint**   | 约束引擎 — 找出领域、角色或争论真正被哪些硬/软/自设约束框住                                                  |
| **zx-grill-me**     | 拷问我 — 对计划、方案或设计做高压追问，一次只问一个关键问题，并给出推荐答案                                  |
| **zx-is**           | 本质提炼器 — 把目标压成“谁或什么从 A 到 B”的最小状态变化                                                     |
