...
**强制 NATIVE 模式。** 本 workflow 是纯 skill 管道（zx-paper →
zx-card），不需要 Algorithm 的七步流程。直接按下方执行步骤调用 skill，不走 OBSERVE/THINK/PLAN/BUILD/EXECUTE/VERIFY/LEARN。
...
调用 Skill tool 执行 `zx-paper`，传入该论文的来源。等待完成，获得生成的 org 文件路径。
...
- zx-paper 和 zx-card 各自的质量标准、红线、品味准则不变
