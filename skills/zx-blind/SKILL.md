---
name: zx-blind
description: 盲区扫描——读昨天你与 AI 的全部对话，照出暴露的思维盲区（不是不懂的知识，是让某类真相一直看不见的思维习惯），再从微信读书挑一本书的一章精准补上，落成一篇完整分析笔记。Use when user says '扫盲区', '盲区', '照盲区', '看看我的思维盲区', '我昨天想漏了什么', 'blind spot', 'zx-blind', '/zx-blind', or wants yesterday's AI conversations analyzed for cognitive blind spots plus a targeted weread chapter to fill them. 可选参数：传一个日期(YYYY-MM-DD)扫那天而非昨天。NOT FOR 纵向深钻一个观点（用 zx-think）、一个领域降秩（用 zx-rank）、陪读一篇文本（用 zx-read）、找一个领域的约束（用 zx-constraint）。
user_invocable: true
version: '1.0.0'
---
...
3. **单一取景框**——一整天只调用一两副 f（逢事就"约束""进化""博弈"），没试过别的镜子。这是最隐蔽的盲区：越趁手的 f，越挡住别的 f 能照见的东西。正因为默认，才最该查今天是不是又只用了其中一副。
...
- **对位 mission**——贴 M0（找新取景框）/ M1（求本质）。盲区若正好挡在用户近期主线上，优先。若当前环境有个人 mission / memory / notes，可读取确认；没有就只依据当天对话，不硬猜。
...
会话文件优先从当前 Codex/Claude 项目日志中找。常见位置包括 `${CODEX_HOME:-$HOME/.codex}/projects/**/*.jsonl`、`$HOME/.claude/projects/**/*.jsonl`、`$HOME/.agents/projects/**/*.jsonl`。每行一条消息（type=user / assistant / system）。只取真人 user 发言，滤掉工具回显和子 agent 噪声：

```bash
target=<上一步的日期>
out=/tmp/zx-blind-${target}.txt
: > "$out"
# 只取顶层会话文件（排除 subagents/workflows），只留真人纯文本
for root in "${CODEX_HOME:-$HOME/.codex}/projects" "$HOME/.claude/projects" "$HOME/.agents/projects"; do
  [ -d "$root" ] || continue
  rg -l "\"timestamp\":\"${target}" "$root" 2>/dev/null | grep -vE 'subagents/|workflows/' >> "${out}.files"
done
sort -u "${out}.files" 2>/dev/null | while read -r f; do
...
grep -vE 'tool_use_id|system-reminder|caveat|Caveat|local-command|command-name|command-message|<task-notification>|zx-blind|扫盲区' "$out" > "${out}.f" && mv "${out}.f" "$out"
wc -c "$out"
```
...
```bash
bun Tools/WeReadWebUrl.ts "<bookId>" "<chapterUid>"
```
...
```bash
bun Tools/WeReadWebUrl.ts "<bookId>"
```
...
如果用户另有每日自动复盘或早信，zx-blind 是**随手调**的分析版——同一套「读昨天 -> 看盲区 -> weread 选章」的底子，但更系统、专照盲区、分析写得更足，而且能扫任意一天（传日期参数）。想主动查某天的盲区、要一篇能存档的分析，用 zx-blind。
